function TradingStastics = Backtest701819606(DataTable, WindowsLength, UpperBandFactor, LowerBandFactor, InvestmentAmount, StartingYear, StartegyNo, TradingSize)
%%  Input Variables
%   WindowsLength:          number of days/bars used to calculate moving
%   average
%   UpperBandFactor:        UpperFactor for Bollinger Bands
%   LowerBandFactor:        LowerFactor for Bollinger Bands
%   InvestmentAmount:       The amount of investment used in trading
%   StartingYear:           Year of starting trading
%   TradingSize:            Size for each trade, no input means all-in.
%   StartegyNo:             1 for Bloomberg Strategy, 2 for the Dr.K's

    %%  Calculating Bollinger Bands
    %   Moving Average Window
    % % % % % % % % % % % % % % % % % % % % % % % KInput: Moving Average Length
    % % % WindowsLength = 252;
    %   if data has one day more before the needed data, PlusOneDay = 1, else 0
    PlusOneDay = 1;
    ...
    MovingStd = movstd(Close, [WindowsLength - 1, 0], 1);
    ...
    %%  Trading Process
    % % % % % % % % % % % % % % % % % % % % % % % KInput: InvestmentAmount
    % % % InvestmentAmount = 100000;


    % % % % % % % % % % % % % % % % % % % % % % % KInput: StartingDay
    %   Start trading at 2007
    % % % StartingYear = 2007;
    StartingDateIndex = find(year(DataTable.Date) == StartingYear, 1);
    switch StartegyNo
        case 1
            try
    ...
    DataTable.Port_Pos_Status(isnan(DataTable.Port_Pos_Status)) = 0;
    ...
    TradingStastics = Statistics701819606(TradeTable, DataTable, InvestmentAmount, StartingDateIndex);
end
